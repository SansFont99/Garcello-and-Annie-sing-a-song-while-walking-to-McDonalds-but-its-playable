...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		mcdonalds = new FlxSprite(-1000, -600).loadGraphic(Paths.image('b6bcf15451c3fac02cfe139fd4749a56', 'shared'));
		mcdonalds.antialiasing = ClientPrefs.globalAntialiasing;
		mcdonalds.updateHitbox();
		add(mcdonalds);
		...
		annieDance = new FlxSprite(400, 0);
		annieDance.frames = Paths.getSparrowAtlas('characters/annie', 'shared');
		annieDance.animation.addByPrefix('dance', 'Annie Idle', 24);
		annieDance.setGraphicSize(Std.int(annieDance.width * 1.2));
		annieDance.flipX = true;
		annieDance.antialiasing = ClientPrefs.globalAntialiasing;
		annieDance.shader = swagShader.shader;

		garcelloDance = new FlxSprite(630, -50);
		garcelloDance.frames = Paths.getSparrowAtlas('characters/garcello', 'shared');
		garcelloDance.animation.addByPrefix('dance', 'Garcello Idle', 24);
		garcelloDance.animation.play('dance');
		garcelloDance.flipX = true;
		garcelloDance.antialiasing = ClientPrefs.globalAntialiasing;
		garcelloDance.shader = swagShader.shader;
		add(garcelloDance);
	...
	override function beatHit()
	{
		...
		if(annieDance != null)
			annieDance.animation.play('dance');

		if(garcelloDance != null)
			annieDance.animation.play('dance');
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				case 12:
					deleteCoolText();
				case 13:
					addMoreText('Garcello and Annie');
				case 14:
					addMoreText('sing a song while');
				case 15:
					addMoreText("walking to McDonald's");
				case 16:
					skipIntro();
			}
		}
